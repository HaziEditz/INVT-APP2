...
export function CurrentTripPanel() {
    ...
    return (
      <View style={styles.panel}>
        ...
        <Text style={styles.meta}>
          {meter.distanceKm.toFixed(1)} km · {meter.waiting ? 'waiting' : meter.paused ? 'paused' : 'running'}
        </Text>
        <View style={styles.actions}>
          <Button title="Pause" variant="secondary" onPress={pauseMeter} />
          <Button title="Wait time" variant="secondary" onPress={toggleWaitMeter} />
  ...
  return (
    <ScrollView style={styles.panel} nestedScrollEnabled showsVerticalScrollIndicator={false}>
      ...
      {runningMeter && meter ? (
        <Text style={styles.meterLine}>Meter ${meter.fare.toFixed(2)} · {meter.distanceKm.toFixed(1)} km</Text>
      ) : null}
...
const styles = StyleSheet.create({
  ...
  meterFare: { color: Colors.success, fontSize: 28, fontWeight: '800', marginTop: 4 },
  ...
  meterLine: { color: Colors.success, fontSize: 16, fontWeight: '700', marginTop: 4 },
